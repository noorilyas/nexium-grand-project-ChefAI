// app/api/generate-recipe-from-prompt/route.ts
...
/**
 * Checks if the user's prompt is cooking-related using the OpenAI API.
 * This function is used to filter out unrelated prompts before spending resources
 * on a full recipe generation.
 * @param userPrompt The prompt to check.
 * @returns true if the prompt is cooking-related, false otherwise.
 */
async function isCookingRelatedByAI(userPrompt: string): Promise<boolean> {
    ...
    messages: [
      {
        role: "system",
        content:
          "Your task is to determine if a user's request is asking for a recipe, meal idea, or anything related to cooking. Respond with 'yes' if it is, and 'no' if it is not. Provide only 'yes' or 'no' in your response.",
      },
      {
        role: "user",
  ...
  console.log(`AI Classification for "${userPrompt}": ${response}`);
  return response === "yes";
}
...
export async function POST(request: Request) {
  ...
  if (!userPrompt) {
    return NextResponse.json({ message: "A 'userPrompt' field is required in the request body." }, { status: 400 });
  }

  // Step 1: Check if the prompt is cooking-related
  let isCookingRelated = false;
  try {
    isCookingRelated = await isCookingRelatedByAI(userPrompt);
  } catch (error: any) {
    console.error("Error during cooking-related check:", error);
    // If the check fails, we proceed, assuming it's a valid prompt to not block the user.
    // However, it's good practice to log this error.
  }

  if (!isCookingRelated) {
    return NextResponse.json(
      {
        message: "the prompt is not asking for meal",
      },
      { status: 400 }
    );
  }

  // Define the prompt text to generate the recipe based on the user's prompt
  const recipePromptText = `Generate a detailed and creative recipe in JSON format based on the following user request: "${userPrompt}". 
  Ensure the output is *only* the JSON object, ready for direct parsing. Do not include any markdown backticks or extra text outside the JSON. The JSON should have the following structure and fields, including a section for nutritional information (calories, protein, fat) based on common understanding of ingredients. You must always include approximate values for calories, protein, and fat per serving. Use "N/A" if info is not available or estimable.
  ...
  let rawRecipeText: string | undefined | null;

  try {
    // Call OpenAI to generate the recipe
