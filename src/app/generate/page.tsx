    ...
    <div className={`${inter.variable} font-inter min-h-screen bg-[#FFF8F3] text-[#2F1B12] pt-24 pb-12 px-4 sm:px-6 lg:px-8`}>
      ...
      <main className="max-w-6xl mx-auto">
        <motion.h1
          ...
          className={`${lexend.variable} font-lexend text-5xl font-extrabold text-center mb-6 text-[#2F1B12]`}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <motion.div
            ...
            className="bg-white p-8 rounded-2xl shadow-xl border border-[#FFDAB9] h-fit sticky top-28"
          >
            <h2 className={`${lexend.variable} font-lexend text-3xl font-bold mb-6 text-[#FF7A59] flex items-center`}>
              <ClipboardList className="mr-3 w-7 h-7" /> What's in Your Pantry?
            </h2>
            <form onSubmit={handleSubmit} className="space-y-8">
              <div>
                <Label htmlFor="current-ingredient-input" className="text-lg font-semibold text-[#2F1B12] mb-2 block">
                ...
                <div className="flex gap-2 mb-2">
                  <Input
                    ...
                    className="flex-grow p-3 border border-orange-200 rounded-lg focus:ring-2 focus:ring-[#FF7A59] focus:border-transparent transition-all duration-200 text-base"
                  />
                  <Button
                    type="button"
                ...
                <div className="flex flex-wrap gap-2 mt-2 min-h-[40px] border border-dashed border-orange-200 p-2 rounded-lg">
                  ...
                  {ingredients.map((ingredient, index) => (
                    <Badge
                      key={index}
                      variant="outline"
                      className="bg-[#FF7A59] text-white px-3 py-1.5 rounded-full flex items-center gap-1.5 text-base hover:bg-[#e66549] cursor-pointer"
                    ...
                    >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="h-5 w-5 rounded-full hover:bg-white/30 text-white p-0"
                        onClick={(e) => {
                            e.stopPropagation();
                ...
                <p className="text-sm text-gray-500 mt-1">Add ingredients one by one using the input field above.</p>
              </div>

              {/* Preferences Section */}
              <Separator className="bg-orange-100" />
              <div className="space-y-6">
                <h3 className={`${lexend.variable} font-lexend text-2xl font-bold text-[#2F1B12] flex items-center`}>
                  <Sparkles className="mr-2 w-6 h-6" /> Your Culinary Preferences
                </h3>

                {/* Dietary Restrictions */}
                <div>
                  <Label className="text-lg font-semibold text-[#2F1B12] mb-2 block">Dietary Restrictions</Label>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {commonDietaryOptions.map(option => (
                      <Badge
                        key={option}
                        variant={dietaryRestrictions.includes(option) ? 'default' : 'outline'}
                        className={`cursor-pointer px-4 py-2 text-base ${dietaryRestrictions.includes(option) ? 'bg-[#FF7A59] text-white hover:bg-[#e66549]' : 'border-orange-300 text-gray-700 hover:bg-orange-50'}`}
                  ...
                  <Input
                    ...
                    className="mt-3 p-3 border border-orange-200 rounded-lg focus:ring-2 focus:ring-[#FF7A59] focus:border-transparent transition-all duration-200 text-base"
                  />
                </div>

                {/* Cuisine Preference */}
                <div>
                  <Label className="text-lg font-semibold text-[#2F1B12] mb-2 block">Cuisine Preference</Label>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {commonCuisineOptions.map(option => (
                      <Badge
                        key={option}
                        variant={cuisinePreference.includes(option) ? 'default' : 'outline'}
                        className={`cursor-pointer px-4 py-2 text-base ${cuisinePreference.includes(option) ? 'bg-[#FF7A59] text-white hover:bg-[#e66549]' : 'border-orange-300 text-gray-700 hover:bg-orange-50'}`}
                ...
                {/* Meal Type */}
                <div>
                  <Label className="text-lg font-semibold text-[#2F1B12] mb-2 block">Meal Type</Label>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {commonMealTypeOptions.map(option => (
                      <Badge
                        key={option}
                        variant={mealType.includes(option) ? 'default' : 'outline'}
                        className={`cursor-pointer px-4 py-2 text-base ${mealType.includes(option) ? 'bg-[#FF7A59] text-white hover:bg-[#e66549]' : 'border-orange-300 text-gray-700 hover:bg-orange-50'}`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="servingSize" className="text-lg font-semibold text-[#2F1B12]">Servings</Label>
                    <Input
                      id="servingSize"
                      type="text"
                      value={servingSize}
                      ...
                      className="mt-2 p-3 border border-orange-200 rounded-lg focus:ring-2 focus:ring-[#FF7A59] focus:border-transparent transition-all duration-200 text-base"
                    />
                  </div>
                  <div>
                    <Label htmlFor="cookingTimeValue" className="text-lg font-semibold text-[#2F1B12]">Cook Time</Label>
                    <div className="flex gap-2 mt-2">
                      <Input
                        ...
                        placeholder="e.g., 30"
                        className="w-24 p-3 border border-orange-200 rounded-lg focus:ring-2 focus:ring-[#FF7A59] focus:border-transparent transition-all duration-200 text-base"
                      />
                      <Select value={cookingTimeUnit} onValueChange={setCookingTimeUnit}>
                        <SelectTrigger className="flex-grow p-3 border border-orange-200 rounded-lg focus:ring-2 focus:ring-[#FF7A59] focus:border-transparent text-base">
                ...
                <div>
                  <Label className="text-lg font-semibold text-[#2F1B12] mb-2 block">Difficulty</Label>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {commonDifficultyOptions.map(option => (
                      <Badge
                        key={option.label}
                        variant={difficulty === option.label ? 'default' : 'outline'}
                        className={`cursor-pointer px-4 py-2 text-base flex items-center ${difficulty === option.label ? 'bg-[#FF7A59] text-white hover:bg-[#e66549]' : 'border-orange-300 text-gray-700 hover:bg-orange-50'}`}
              ...
              <Button
                type="submit"
                className="w-full bg-[#FF7A59] hover:bg-[#e66549] text-white text-xl px-8 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center transform hover:-translate-y-1 mt-10"
                disabled={loading}
              >
                {loading ? (
                  <motion.span
                  ...
                  >
                    <ChefHat className="animate-spin-slow-custom mr-3 w-6 h-6" /> Crafting Your Culinary Masterpiece...
                  </motion.span>
                ) : (
                  <>
                    <UtensilsCrossed className="mr-3 w-6 h-6" /> Cook for me!
                  </>
                )}
              </Button>
            </form>
          ...
          >
            <AnimatePresence mode="wait">
              {error && (
                ...
                >
                  <Alert variant="destructive" className="border-red-300 bg-red-50 text-red-800 p-6">
                    <AlertCircle className="h-5 w-5 mr-3" />
                    <AlertTitle className="text-lg font-semibold mb-2">Oops! Something went wrong.</AlertTitle>
              ...
              {loading && !recipe && (
                <motion.div
                  ...
                  className="w-full bg-white p-8 rounded-2xl shadow-xl border border-[#FFDAB9] text-center flex flex-col items-center justify-center min-h-[400px]"
                >
                  <ChefHat className="animate-spin-slow-custom text-[#FF7A59] w-24 h-24 mb-6" />
                  <p className={`${lexend.variable} font-lexend text-2xl font-bold text-[#2F1B12]`}>
                    ChefAI is whipping up something delicious...
                  </p>
                  <p className="text-lg text-gray-600 mt-2">This might take a moment as our AI assistant prepares your recipe and a unique image.</p>
                </motion.div>
              )}
              ...
              {recipe && (
                <motion.div
                  ...
                  className="w-full bg-white p-8 rounded-2xl shadow-xl border border-[#FFDAB9]"
                >
                  <Card className="border-none shadow-none">
                    <CardHeader className="p-0 mb-6">
                      <CardTitle className={`${lexend.variable} font-lexend text-4xl font-extrabold text-[#2F1B12] mb-4 text-center`}>
                    ...
                    <CardContent className="p-0 space-y-8">
                      <div>
                        <h3 className="text-2xl font-semibold mb-4 text-[#FF7A59] flex items-center">
                          <Wheat className="mr-2 w-6 h-6" /> Ingredients
                        </h3>
                      ...
                      <div>
                        <h3 className="text-2xl font-semibold mb-4 text-[#FF7A59] flex items-center">
                          <ClipboardList className="mr-2 w-6 h-6" /> Instructions
                        </h3>
                        <ol className="list-decimal list-inside space-y-3 text-gray-700 text-lg leading-relaxed">
              ...
              {!loading && !error && !recipe && (
                <motion.div
                  ...
                  className="w-full bg-white p-8 rounded-2xl shadow-xl border border-[#FFDAB9] text-center flex flex-col items-center justify-center min-h-[400px]"
                >
                  <Sparkles className="text-orange-300 w-24 h-24 mb-6" />
                  <p className={`${lexend.variable} font-lexend text-2xl font-bold text-[#2F1B12]`}>
